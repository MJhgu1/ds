...
// It uses Dijkstra°Øs two-stack algorithm. For simplicity of coding, however,
...
int evaluate(string tokens) {
	...
	for (int i = 0; i < tokens.length(); i++) {
		...
		if (isdigit(tokens[i])) {
			...
			while (!op_stack.empty()&&priority(op_stack.top())>=priority(tokens[i])) {
				int answer = compute(va_stack, op_stack);
				va_stack.push(answer);
				DPRINT(cout << " va_stack.push: " << answer << endl;);
			}
