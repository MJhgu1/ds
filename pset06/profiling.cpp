	...
	for (int i = STARTING_SAMPLES; i <= n; i += getStep(i)) {
		...
		do {
			   repetitions++;                // count the repetitions for one second
				 copy_n(saved,i,list);                // get a copy of list from saved
...
int main(int argc, char *argv[]) {
  ...
  // replace the following line for dynamic memory allocation by N
	...
	assert(list != nullptr);                    // Use new & N here
	...
	cout << "\n\tquickSort(): already sorted - worst case." << endl;	// reversed sequence
